...
pub fn transaction_enter(label: &str,is_db: bool,) -> Option<_rt::String>{
  unsafe {
    ...
    wit_import(ptr0.cast_mut(), len0, match &is_db { true => 1, false => 0 }, ptr1);
...
pub unsafe fn _export_start_cabi<T: Guest>() {#[cfg(target_arch="wasm32")]
_rt::run_ctors_once();T::start();
}
pub trait Guest {
  fn start();
}
...
A\x0d\x01@\0\0s\x03\0\x09task-name\x01\0\x03\0\x09task-data\x01\0\x01@\x01\x07me\
ssages\x01\0\x03\0\x05abort\x01\x01\x01ks\x01@\x02\x05labels\x05is-db\x7f\0\x02\x03\
\0\x11transaction-enter\x01\x03\x01@\x01\x04datas\x01\0\x03\0\x10transaction-exi\
t\x01\x04\x03\0\x05print\x01\x04\x01@\0\x01\0\x04\0\x05start\x01\x05\x04\x01\x17\
durable:core/core@1.0.0\x04\0\x0b\x0a\x01\0\x04core\x03\0\0\0G\x09producers\x01\x0c\
